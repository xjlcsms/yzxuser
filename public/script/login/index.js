...
(function(){
    ...
    var captcha_html = '<div class="input-wrapper">\
        <input class="form-control" type="text" name="captcha" value="" placeholder="验证码" maxlength="4" tabindex="3" autocomplete="off">\
        <a href="javascript:;" class="captcha"><img src="/i/captcha/?channel=login" alt="图片验证码" title="更换验证码" width="100" height="40"></a>\
    </div>';
    var _reg = {
    ...
    $form.submit(function(){
        ...
        checkFileds(fields, function() {
            ...
            $.post("/login/i/",param,function(json){
                if (json['status']) {//登陆成功
                    window.location.href = json.data.url;
                } else {
                    if (json['data'] && json['data']['using_captcha'] && !$captcha.length) {
                        $password.parents('.input-wrapper').after(captcha_html);
                        $captcha = $form.find('input[name="captcha"]');
                    }
                    $form.find('.captcha img').attr('src','/i/captcha/?channel=login&v=' + (+new Date));
...
(function(){
    ...
    //切换验证码
    $form.on('click', '.captcha img', function() {
        this.src = '/i/captcha/?channel=sms&v=' + (+new Date);
    });
    //发送短信
    (function() {
        var time = 0, 
    ...
    $form.submit(function(){
        ...
        checkFileds(fields, function() {
            if(!$('#read').prop('checked')){
                ...
                $.post('/reg/doreg/',param,function(json){
                    if (json['status']) {//登陆成功
                        delCookie('form_time');
                        delCookie('form_user');
