...
class Browser
{
    ...
    const BROWSER_NETPOSITI60VE = 'NetPositive'; // http://en.wikipedia.org/wiki/NetPositive (DEPRECATED)
    const BROWSER_PHOENIX = 'Phoenix'; // http://en.wikipedia.org/wiki/History_of_Mozilla_Firefox (DEPRECATED)
    //中国特有的
    const BROWSER_IQIYI = 'IqiyiApp';
    const BROWSER_VIVALDI = 'Vivaldi';
    const BROWSER_KINDLE = 'kindle';
    const BROWSER_360 = '360 se(ee)';
    const BROWSER_UC = 'uc browser';
    const BROWSER_QQBROWSER = 'QQBrowser';
    const BROWSER_QQ = 'Qq';
    const BROWSER_BAIDU = 'BIDUBrowser';
    const BROWSER_SOGOU = 'Sogou';
    const BROWSER_LBBROWSER = 'LBBROWSER';
    const BROWSER_2345EXPLORER = '2345Explorer';
    const BROWSER_THEWORLD = 'TheWorld';
    const BROWSER_XIAOMI = 'MiuiBrowser';
    const BROWSER_QUARK = 'Quark';
    const BROWSER_QIYU = 'Qiyu';
    const BROWSER_WECHAT = 'Wechat';
    const BROWSER_TAOBAO = 'Taobao';
    const BROWSER_ALIPAY = 'Alipay';
    const BROWSER_WEIBO = 'Weibo';
    const BROWSER_SUNING = 'SNEBUY-APP';
    const BROWSER_DOUBAN = 'Douban';
    ...
    const PLATFORM_WINDOWSPHONE = 'Wp';
 ...
 /**
    ...
    {
        if (stripos($this->_agent, 'QQ/') !== false) {
            preg_match('/^.*QQ\/([\d.]+).*$/', $this->_agent,$matches);
            if (isset($matches[1])) {
                $this->setVersion($matches[1]);
            } else {
                $this->setVersion(self::VERSION_UNKNOWN);
    ...
    protected function checkQQBrowser()
    {
        if (stripos($this->_agent, 'QQBrowser') !== false) {
            preg_match('/^.*QQBrowser\/([\d.]+).*$/', $this->_agent,$matches);
            if (isset($matches[1])) {
                $this->setVersion($matches[1]);
            } else {
                $this->setVersion(self::VERSION_UNKNOWN);
            }
            $this->setBrowser(self::BROWSER_QQBROWSER);
            return true;
        }
        return false;
    }
     ...
     * @return bool True if the browser is Android otherwise false
     */
    protected function check360Browser()
    {
        if (stripos($this->_agent, '360EE') !== false || stripos($this->_agent, '360SE') !== false) {
            $this->setVersion(self::VERSION_UNKNOWN);
     ...
     * @return bool True if the browser is Android otherwise false
     */
    protected function checkUCBrowser()
    {
        if (stripos($this->_agent, 'UC') !== false || stripos($this->_agent, 'UBrowser') !== false ) {
            preg_match('/^.*UC?Browser\/([\d.]+).*$/', $this->_agent,$matches);
            if (isset($matches[1])) {
                $this->setVersion($matches[1]);
            } else {
                $this->setVersion(self::VERSION_UNKNOWN);
     ...
     * @return bool True if the browser is Android otherwise false
     */
    protected function checkBaiduBrowser()
    {
        if (stripos($this->_agent, 'Baidu') !== false || stripos($this->_agent, 'BIDUBrowser') !== false) {
            preg_match('/^.*BIDUBrowser[\s\/]([\d.]+).*$/', $this->_agent,$matches);
            if (isset($matches[1])) {
                $this->setVersion($matches[1]);
            } else {
                $this->setVersion(self::VERSION_UNKNOWN);
            }
            $this->setBrowser(self::BROWSER_BAIDU);
            return true;
        }
        return false;
    }
     ...
     * @return bool True if the browser is Android otherwise false
     */
    protected function checkSogouBrowser()
    {
        if (stripos($this->_agent, 'MetaSr') !== false || stripos($this->_agent, 'Sogou') !== false) {
            preg_match('/^.*SE ([\d.X]+).*$/', $this->_agent,$matches1);
            preg_match('/^.*SogouMobileBrowser\/([\d.]+).*$/', $this->_agent,$matches2);
            if (isset($matches1[1]) || $matches2[1]) {
                $this->setVersion(isset($matches1[1])?$matches1[1]:$matches2[1]);
            } else {
                $this->setVersion(self::VERSION_UNKNOWN);
            }
            $this->setBrowser(self::BROWSER_SOGOU);
            return true;
        }
        return false;
    }
    ...
    protected function checkWechat()
    {
        if (stripos($this->_agent, 'MicroMessenger') !== false) {
            preg_match('/^.*MicroMessenger\/([\d.]+).*$/', $this->_agent,$matches);
            if (isset($matches[1])) {
                $this->setVersion($matches[1]);
            } else {
                $this->setVersion(self::VERSION_UNKNOWN);
     ...
     * @return bool True if the browser is Android otherwise false
     */
    protected function checkWeibo()
    {
        if (stripos($this->_agent, 'Weibo') !== false) {
            preg_match('/^.*weibo__([\d.]+).*$/', $this->_agent,$matches);
            if (isset($matches[1])) {
                $this->setVersion($matches[1]);
            } else {
                $this->setVersion(self::VERSION_UNKNOWN);
            }
            $this->setBrowser(self::BROWSER_WEIBO);
            return true;
        }
        return false;
    }
     ...
     * @return bool True if the browser is Android otherwise false
     */
    protected function checkDouban()
    {
        if (stripos($this->_agent, 'com.douban.frodo') !== false) {
            preg_match('/^.*com\.douban\.frodo\/([\d.]+).*$/', $this->_agent,$matches);
            if (isset($matches[1])) {
                $this->setVersion($matches[1]);
            } else {
                $this->setVersion(self::VERSION_UNKNOWN);
            }
            $this->setBrowser(self::BROWSER_DOUBAN);
            return true;
        }
        return false;
    }
     ...
     * @return bool True if the browser is Android otherwise false
     */
    protected function checkTaobao()
    {
        if (stripos($this->_agent, 'AliApp(TB') !== false) {
            preg_match('/^.*AliApp\(TB\/([\d.]+).*$/', $this->_agent,$matches);
            if (isset($matches[1])) {
                $this->setVersion($matches[1]);
            } else {
                $this->setVersion(self::VERSION_UNKNOWN);
            }
            $this->setBrowser(self::BROWSER_TAOBAO);
            return true;
        }
        return false;
    }
     ...
     * @return bool True if the browser is Android otherwise false
     */
    protected function checkAlipay()
    {
        if (stripos($this->_agent, 'AliApp(AP') !== false) {
            preg_match('/^.*AliApp\(AP\/([\d.]+).*$/', $this->_agent,$matches);
            if (isset($matches[1])) {
                $this->setVersion($matches[1]);
            } else {
                $this->setVersion(self::VERSION_UNKNOWN);
            }
            $this->setBrowser(self::BROWSER_ALIPAY);
            return true;
        }
        return false;
    }
